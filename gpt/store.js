    ...
    if (products.length > 0) // if has data
    {
        products.forEach(product => {
            ...
            newProduct.innerHTML =
                ...
                <button class="addCart" onclick="addToCart(${product.id}, '${product.name}', ${product.price}, '${product.image}')">Add To Cart</button>`;
...
listProductHTML.addEventListener('click', (event) => {
    ...
    if (positionClick.classList.contains('addCart')) {
        let id_product = parseInt(positionClick.parentElement.dataset.id);
